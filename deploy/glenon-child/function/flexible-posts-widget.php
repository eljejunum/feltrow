...
class DPE_Flexible_Posts_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array( 'description' => __( 'Display posts as widget items', 'glenon_wp' ) ) // Args
		);
		
		// $this->directory	= plugins_url( '/', __FILE__ );
		
		// $this->register_hooks();							// Register actions & filters
		$this->register_sns( FUNCTION_DIR ); 	// Register styles & scripts
		...
		if ( defined("WP_ADMIN") && WP_ADMIN ) {
			if ( 'widgets.php' == $pagenow ) {
				...
				wp_localize_script( 'flexible-posts-widget', 'objectL10n', array(
					'gettingTerms' => __( 'Getting terms...', 'glenon_wp' ),
					'selectTerms' => __( 'Select terms:', 'glenon_wp' ),
					'noTermsFound' => __( 'No terms found.', 'glenon_wp' ),
				) );
			}
			if ( 'post.php' == $pagenow ) {
				wp_enqueue_style( 'flexible-posts-widget' );
   ...
   /**
    ...
    function widget($args, $instance) {	
		...
		if ( empty($template) ) $template = 'temp-widget.php';
		...
		include( FUNCTION_DIR . 'views/'.$instance['template'] );
    ...
    function update( $new_instance, $old_instance ) {
		...
		$this->orderbys		= array(
			'date'		 	=> __('Publish Date', 'glenon_wp'),
			'title'			=> __('Title', 'glenon_wp'),
			'menu_order'	=> __('Menu Order', 'glenon_wp'),
			'ID'			=> __('Post ID', 'glenon_wp'),
			'author'		=> __('Author', 'glenon_wp'),
			'name'	 		=> __('Post Slug', 'glenon_wp'),
			'comment_count'	=> __('Comment Count', 'glenon_wp'),
			'rand'			=> __('Random', 'glenon_wp'),
		);
		$this->orders		= array(
			'ASC'	=> __('Ascending', 'glenon_wp'),
			'DESC'	=> __('Descending', 'glenon_wp'),
		);
	...
	function get_temp_templates(){
		$templates = array();

		$files = glob(get_template_directory().'/function/views/temp*.php');
		foreach($files as $file){
			$templates[] = basename($file);
		}
		$files = glob(get_stylesheet_directory().'/function/views/temp*.php');
		foreach($files as $file){
			$templates[] = basename($file);
		}
    ...
    function form( $instance ) {
		...
		$instance = wp_parse_args( (array) $instance, array(
			...
			'thumbnail' => '1',
			'thumbsize' => '',
			'template'	=> 'temp-widget.php',
			// 'cur_tab'	=> '0',
		) );
		
		extract( $instance );

		$templates = $this->get_temp_templates();
		
		include( FUNCTION_DIR . 'views/admin-flexi.php' );
		
		// include( $this->getTemplateHierarchy( 'admin-flexi' ) );
	...
	public function register_sns() {
		wp_register_script( 'flexible-posts-widget', FUNCTION_URL . 'js/admin.js', array('jquery', 'jquery-ui-tabs' ), DPE_FP_Version, true );
		wp_register_script( 'dpe-fp-for-pb', FUNCTION_URL . 'js/admin-for-pb.js', array('jquery', 'jquery-ui-tabs' ), DPE_FP_Version, true );
		wp_register_style( 'flexible-posts-widget', FUNCTION_URL . 'css/admin.css', array(), DPE_FP_Version );
	}
	...
	public function terms_checklist( $term ) {
		...
		if( empty($terms) ) { 
			$output = '<p>' . __( 'No terms found.', 'glenon_wp' ) . '</p>';
		} else {
