...
<div id="panels">
	...
	<div id="add-to-panels">
		<button class="panels-add tooltip" data-tooltip="<?php esc_attr_e('Add Widget', 'glenon_wp') ?>"><?php _e('Add Widget', 'glenon_wp') ?></button>
		<button class="grid-add tooltip" data-tooltip="<?php esc_attr_e('Add Columns', 'glenon_wp') ?>"><?php _e('Add Columns', 'glenon_wp') ?></button>
		<?php if(!empty($layouts)) : ?>
			<button class="prebuilt-set tooltip" data-tooltip="<?php esc_attr_e('Prebuilt Layouts', 'glenon_wp') ?>"><?php _e('Prebuilt Layouts', 'glenon_wp') ?></button>
		<?php endif; ?>
		<div class="clear"></div>
	</div>
	...
	<div id="panels-dialog" data-title="<?php esc_attr_e('Add New Widget', 'glenon_wp') ?>" class="panels-admin-dialog">
	...
	<div id="grid-add-dialog" data-title="<?php esc_attr_e('Add Columns', 'glenon_wp') ?>" class="panels-admin-dialog">
		<p><label><strong><?php _e('Columns', 'glenon_wp') ?></strong></label></p>
		<p><input type="text" id="grid-add-dialog-input" name="column_count" class="small-text" value="3" /></p>
	</div>
	
	<?php if(!empty($layouts)) : ?>
		<div id="grid-prebuilt-dialog" data-title="<?php esc_attr_e('Insert Prebuilt Page Layout', 'glenon_wp') ?>" class="panels-admin-dialog">
			<p><label><strong><?php _e('Page Layout', 'glenon_wp') ?></strong></label></p>
			<p>
				<select type="text" id="grid-prebuilt-input" name="prebuilt_layout" style="width:580px;" placeholder="<?php esc_attr_e('Select Layout', 'glenon_wp') ?>" >
					...
					<?php foreach($layouts as $id => $data) : ?>
						<option id="panel-prebuilt-<?php echo esc_attr($id) ?>" data-layout-id="<?php echo esc_attr($id) ?>" class="prebuilt-layout">
							<?php echo isset($data['name']) ? $data['name'] : __('Untitled Layout', 'glenon_wp') ?>
						</option>
					<?php endforeach; ?>
				</select>
			</p>
