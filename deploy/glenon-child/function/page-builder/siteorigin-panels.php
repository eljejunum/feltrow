...
include (PAGEBUILDER_DIR .'inc/widgets.php');

// Theme Options
include (PAGEBUILDER_DIR .'inc/options.php');
...
function siteorigin_panels_admin_menu(){
	...
	add_theme_page(
		__('Custom Home Page Builder', 'glenon_wp'),
		__('Home Page', 'glenon_wp'),
		'edit_theme_options',
		'so_panels_home_page',
...
function siteorigin_panels_render_admin_home_page(){
	add_meta_box( 'so-panels-panels', __( 'Page Builder', 'glenon_wp' ), 'siteorigin_panels_metabox_render', 'appearance_page_so_panels_home_page', 'advanced', 'high' );
	include (PAGEBUILDER_DIR .'tpl/admin-home-page.php');
}
...
function siteorigin_panels_metaboxes() {
	foreach(siteorigin_panels_setting('post-types') as $type){
		add_meta_box( 'so-panels-panels', __( 'Page Builder', 'glenon_wp' ), 'siteorigin_panels_metabox_render', $type, 'advanced', 'high' );
	}
}
...
function siteorigin_panels_metabox_render( $post ) {
	include (PAGEBUILDER_DIR .'tpl/metabox-panels.php');
}
...
function siteorigin_panels_admin_enqueue_scripts($prefix) {
	...
	if ( ( $screen->base == 'post' && in_array( $screen->id, siteorigin_panels_setting('post-types') ) ) || $screen->base == 'appearance_page_so_panels_home_page') {
		...
		wp_enqueue_script( 'so-undomanager', PAGEBUILDER_URL . 'js/undomanager.min.js', array( ), 'fb30d7f' );

		wp_enqueue_script( 'so-panels-admin', PAGEBUILDER_URL . 'js/panels.admin.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-panels', PAGEBUILDER_URL . 'js/panels.admin.panels.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-grid', PAGEBUILDER_URL . 'js/panels.admin.grid.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-prebuilt', PAGEBUILDER_URL . 'js/panels.admin.prebuilt.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-tooltip', PAGEBUILDER_URL . 'js/panels.admin.tooltip.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-media', PAGEBUILDER_URL . 'js/panels.admin.media.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		
		wp_enqueue_script( 'so-panels-chosen', PAGEBUILDER_URL . 'js/chosen/chosen.jquery.min.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		...
		wp_localize_script( 'so-panels-admin', 'panels', array(
			...
			'i10n' => array(
				'buttons' => array(
					'insert' => __( 'Insert', 'glenon_wp' ),
					'cancel' => __( 'cancel', 'glenon_wp' ),
					'delete' => __( 'Delete', 'glenon_wp' ),
					'done' => __( 'Done', 'glenon_wp' ),
					'undo' => __( 'Undo', 'glenon_wp' ),
					'add' => __( 'Add', 'glenon_wp' ),
				),
				'messages' => array(
					'deleteColumns' => __( 'Columns deleted', 'glenon_wp' ),
					'deleteWidget' => __( 'Widget deleted', 'glenon_wp' ),
					'confirmLayout' => __( 'Are you sure you want to load this layout? It will overwrite your current page.', 'glenon_wp' ),
					'editWidget' => __('Edit %s Widget', 'glenon_wp')
				),
			),
		) );
...
function siteorigin_panels_admin_enqueue_styles() {
	...
	if ( in_array( $screen->id, siteorigin_panels_setting('post-types') ) || $screen->base == 'appearance_page_so_panels_home_page') {
		wp_enqueue_style( 'so-panels-jquery-ui', PAGEBUILDER_URL . 'css/jquery-ui-theme.css' );
		wp_enqueue_style( 'so-panels-admin', PAGEBUILDER_URL . 'css/panels-admin.css' );
		wp_enqueue_style( 'so-panels-chosen', PAGEBUILDER_URL . 'js/chosen/chosen.css' );
...
function siteorigin_panels_add_help_tab($prefix) {
	...
	if(
		...
		$screen->add_help_tab( array(
			...
			'title' => __( 'Page Builder', 'glenon_wp' ), //unique visible title for the tab
...
function siteorigin_panels_add_help_tab_content(){
	include (PAGEBUILDER_DIR  . 'tpl/help.php');
}
...
function siteorigin_panels_the_widget( $widget, $instance, $grid, $cell, $panel, $is_first, $is_last ) {
	...
	$the_widget->widget( array(
		'before_widget' => '<div class="' . esc_attr( implode( ' ', $classes ) ) . '" id="panel-' . $grid . '-' . $cell . '-' . $panel . '"><div>',
...
function siteorigin_panels_admin_bar_menu($admin_bar){
	...
	if( ( $wp_query->is_home() && $wp_query->is_main_query() ) || siteorigin_panels_is_home() ){
		...
		$admin_bar->add_node(array(
			'id' => 'edit-home-page',
			'title' => __('Edit Home Page', 'glenon_wp'),
...
// function siteorigin_panels_siteorigin_themes_tab($suffix){
	// if( ($suffix == 'theme-install.php' || $suffix == 'themes.php') && !wp_script_is('siteorigin-admin-tab') ){
		// wp_enqueue_script('siteorigin-themes-tab', PAGEBUILDER_URL . 'js/siteorigin.tab.min.js', array('jquery'), SITEORIGIN_PANELS_VERSION);
		// wp_localize_script('siteorigin-themes-tab', 'siteoriginAdminTab', array(
			// 'text' => __('SiteOrigin Themes', 'glenon_wp'),
...
function siteorigin_panels_enqueue_styles(){
	wp_enqueue_style('siteorigin-panels', PAGEBUILDER_URL . 'css/front.css', array(), SITEORIGIN_PANELS_VERSION );
}
...
function siteorigin_panels_cloned_page_layouts($layouts){
	...
	foreach($pages as $page){
		...
		$name =  empty($page->post_title) ? __('Untitled', 'glenon_wp') : $page->post_title;
		if($page->post_status != 'publish') $name .= ' ( ' . __('Unpublished', 'glenon_wp') . ' )';
		
		$layouts['post-'.$page->ID] = wp_parse_args(
			array(
				'name' => sprintf(__('Clone Page: %s', 'glenon_wp'), $name )
			),
			$panels_data
		);
	}
