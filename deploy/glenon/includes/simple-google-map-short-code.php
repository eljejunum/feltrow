	    ...
	    <script type="text/javascript">
			...
			function pw_run_map_<?php echo $map_id ; ?>(){
				...
				var map_options = {
					...
					scrollwheel : <?php echo esc_attr( $atts['scroll'] ); ?>
				}
    ...
    if ($force_refresh || $coordinates === false) {
		...
		if ( $response['response']['code'] == 200 ) {
			...
			if ( $data->status === 'OK' ) {
			  	...
			  	return __( 'No location found for the entered address.', 'glenon_wp' );
			} elseif( $data->status === 'INVALID_REQUEST' ) {
			   	return __( 'Invalid request. Did you enter an address?', 'glenon_wp' );
			} else {
				return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'glenon_wp' );
			}

		} else {
		 	return __( 'Unable to contact Google API service.', 'glenon_wp' );
		}
